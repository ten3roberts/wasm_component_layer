    ...
    fn expand_variant(
        ...
        for (index, VariantCase { ident, ty, .. }) in cases.iter().enumerate() {
            ...
            lowers.extend(quote!(#pattern => {
                #internal::map_maybe_uninit!(dst.tag).write(wasm_component_layer::ValRaw::u32(#index_u32));
                unsafe {
    ...
    fn expand_variant(
        ...
        for (index, VariantCase { attrs, ident, ty }) in cases.iter().enumerate() {
            ...
            if let Some(ty) = ty {
                ...
                lower_payload_case_declarations
                    .extend(quote!(#ident: [wasm_component_layer::ValRaw; 0],));
            }
        }
        ...
        let expanded = quote! {
            ...
            pub struct #lower<#lower_payload_generic_params> {
                tag: wasm_component_layer::ValRaw,
